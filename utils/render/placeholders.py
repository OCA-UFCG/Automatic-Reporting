...
_CAMPOS_VACINA = {"vacina_meta", "vacina_nao_meta"}
_LITERAIS_VACINA = ("todas", "nenhuma")
...
def _avaliar_condicao_vacina(expressao: str, contexto: dict) -> bool | None:
    ...
    for parte in _CONJUNCAO.split(expressao):
        ...
        if campo not in _CAMPOS_VACINA:
            if not _avaliar_condicao_editorial(matches, parte, contexto):
                return False
            continue
        trecho = parte[matches[0].end():].casefold()
        literal = next((lit for lit in _LITERAIS_VACINA if lit in trecho), None)
        if literal is None:
            return None
